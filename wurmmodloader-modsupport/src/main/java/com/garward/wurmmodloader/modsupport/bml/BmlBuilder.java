...
public class BmlBuilder {
	...
	/**
	 * Creates a radio button node builder with explicit group and ID.
	 *
	 * <p>A radio button is part of a group where only one button can be selected at a time.
	 * Use this method when you need to specify both the group name and a unique ID for each option.</p>
	 *
	 * <p>This is the recommended method for creating radio button groups, as it allows you to
	 * have multiple radio groups in the same form by using different group names.</p>
	 *
	 * <h3>Usage Example:</h3>
	 * <pre>{@code
	 * // Create a radio group for color selection
	 * BmlNodeBuilder red = BmlBuilder.radioInGroup("colorGroup", "red", "Red");
	 * BmlNodeBuilder blue = BmlBuilder.radioInGroup("colorGroup", "blue", "Blue");
	 * BmlNodeBuilder green = BmlBuilder.radioInGroup("colorGroup", "green", "Green");
	 * }</pre>
	 *
	 * @param group the group name (all radio buttons with the same group name are mutually exclusive)
	 * @param id the unique identifier for this specific radio button option
	 * @param text the radio button label text
	 * @return a new {@code BmlNodeBuilder} configured as a radio button with group
	 * @since 1.0.0
	 */
	public static BmlNodeBuilder radioInGroup(String group, String id, String text) {
		return BmlNodeBuilder.builder("radio")
