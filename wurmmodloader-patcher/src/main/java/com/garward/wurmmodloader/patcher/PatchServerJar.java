package com.garward.wurmmodloader.patcher;
...
public class PatchServerJar {
	...
	private void run() throws CannotCompileException, IOException {
		...
		Path serverJarBak = Paths.get("server.jar.bak");
		Path loaderJar = Paths.get("modlauncher.jar");

		// Create backup of original server.jar if it doesn't exist
		if (!Files.exists(serverJarBak) && Files.exists(serverJar)) {
			Files.copy(serverJar, serverJarBak);
			logger.info("Created backup: server.jar.bak");
		}
		...
		try (FileSystem serverFS = FileSystems.newFileSystem(URI.create("jar:" + serverJar.toUri()), new HashMap<>()); FileSystem loaderFS = FileSystems.newFileSystem(URI.create("jar:" + loaderJar.toUri()), new HashMap<>())) {
			...
			Path loaderFile = loaderFS.getPath("com/garward/wurmmodloader/serverlauncher/PatchedLauncher.class");
