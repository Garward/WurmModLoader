...
public class ProxyServerHook extends ServerHook {
	...
	public static com.garward.wurmmodloader.api.events.skill.SkillGainMultiplierEvent fireSkillGainMultiplierEvent(
			com.wurmonline.server.skills.Skill skill,
			double check,
			double power,
			double learnMod,
		...
		return getInstance().fireSkillGainMultiplier(
	...
	/**
	 * Fires ShopDiffEvent from {@code Trade.addShopDiff(long)}.
	 * Returns the (possibly mutated) money value to be applied.
	 */
	public static long fireShopDiffEvent(Object trade, long money, long currentShopDiff) {
		try {
			return getInstance().fireShopDiff(
				(com.wurmonline.server.items.Trade) trade, money, currentShopDiff);
		} catch (Throwable t) {
			java.util.logging.Logger.getLogger(ProxyServerHook.class.getName())
				.log(java.util.logging.Level.WARNING, "Failed to fire ShopDiffEvent", t);
			return money;
		}
	}

	/**
	 * Fires MountSpeedPercentEvent from {@code
	 * Creature.getMountSpeedPercent(boolean)}. Returns a {@link Float}
	 * override or {@code null} if no listener overrode the value (caller
	 * should then run vanilla logic).
	 *
	 * <p>Returned as {@code Object} so the patched method can read it as
	 * {@code (Float)} without requiring an additional import in the
	 * generated bytecode.</p>
	 */
	public static Object fireMountSpeedPercentEvent(Object creature, boolean mounting) {
		try {
			return getInstance().fireMountSpeedPercent(
				(com.wurmonline.server.creatures.Creature) creature, mounting);
		} catch (Throwable t) {
			java.util.logging.Logger.getLogger(ProxyServerHook.class.getName())
				.log(java.util.logging.Level.WARNING, "Failed to fire MountSpeedPercentEvent", t);
			return null;
		}
	}
